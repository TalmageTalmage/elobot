...
const redTeam = require("./redSameTeamWin.js")





let redCompare = (message, eloDiff) => {
    con.query("SELECT * FROM openLobbies WHERE channelID = ?", message.channel.id, (error, game) => {
        con.query("SELECT * FROM ?? WHERE TEAM = 'red'", message.channel.id, (error, rData) => {
            con.query("SELECT * FROM ?? WHERE TEAM = 'blue'", message.channel.id, (error, bData) => {
