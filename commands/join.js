                            ...
                            con.query("SELECT * FROM leaderboards WHERE gameType=? AND guildID=? AND playerID=?", [gameType[0].gameType, message.guild.id, message.author.id], (error, leaderData) => {
                                if (leaderData.length == 0) {
                                    if (args[0] === "capt") {
                                        ...
                                        con.query("INSERT INTO ?? SET ?", [message.channel.id, { id: message.author.id, username: message.author.username, elo: 1000, team: "", capt: true }])
                                    }
                                    else {
                                        con.query("INSERT INTO ?? SET ?", [message.channel.id, { id: message.author.id, username: message.author.username, elo: 1000, team: "", capt: false }])
                                    ...
                                    if (args[0] === "capt") {
                                        con.query("INSERT INTO ?? SET ?", [message.channel.id, { id: message.author.id, username: message.author.username, elo: leaderData[0].elo, team: "", capt: true }])
                                    }
                                    else {
                                        con.query("INSERT INTO ?? SET ?", [message.channel.id, { id: message.author.id, username: message.author.username, elo: leaderData[0].elo, team: "", capt: false }])
